...
var e = Object.create, t = Object.defineProperty, n = Object.getOwnPropertyDescriptor, r = Object.getOwnPropertyNames, i = Object.getPrototypeOf, a = Object.prototype.hasOwnProperty, o = (e, t) => () => (t || (e((t = { exports: {} }).exports, t), e = null), t.exports), s = (e, i, o, s) => {
	...
	value: n,
	enumerable: !0
}) : a, n)), l = "host", u = "queue/data", d = "queue/join", f = "upload", p = "login", m = "config", h = "info", ee = "runtime", te = "sleeptime", ne = "heartbeat", re = "component_server", ie = "reset", ae = "cancel", g = "app_id", oe = "This application is currently busy. Please try again. ", _ = "Connection errored out. ", v = "Could not resolve app config. ", y = "Could not get space status. ", se = "Could not get API info. ", b = "Space metadata could not be loaded. ", x = "Invalid URL. A full URL path is required.", ce = "Not authorized to access this space. ", S = "Invalid credentials. Could not login. ", le = "Login credentials are required to access this space.", C = "Could not access this app (received a 401 response). If it is a private Hugging Face Space, pass a valid Hugging Face token to the `token` option of `Client.connect`. You can generate a token at https://huggingface.co/settings/tokens.", ue = (e, t) => `Space "${e}" could not be accessed (received a ${t} response from the Hugging Face API). Check that the Space name is spelled correctly and that the Space exists. If the Space is private, pass a valid Hugging Face token to the \`token\` option of \`Client.connect\`. You can generate a token at https://huggingface.co/settings/tokens.`, de = "No API information is available for this app. This can happen when the app's `/info` endpoint cannot be reached, or when the app is running a legacy version of Gradio that is not supported by this client. ", fe = "This app appears to be running a legacy version of Gradio (3.x or earlier) that communicates over WebSockets, which is not supported by this version of @gradio/client. Please upgrade the app to a newer version of Gradio, or connect to it with @gradio/client version 0.x.", pe = "File system access is only available in Node.js environments", w = "Root URL not found in client config", me = "Error uploading file";
//#endregion
...
async function _e(e, t = !0) {
	...
	if (typeof window < "u" && window.gradio_config && location.origin !== "http://localhost:9876") {
		if (t && window.gradio_config.current_page && (e = e.substring(0, e.lastIndexOf("/"))), window.gradio_config.dev_mode || typeof window < "u" && window?.BUILD_MODE === "dev") {
			let t = P(e, this.deep_link ? m + "?deep_link=" + this.deep_link : m), r = await D(await this.fetch(t, {
				headers: n,
		...
		return r.root = ge(r.root, location.href, r.is_colab), r;
	} else if (e) {
		let t = P(e, this.deep_link ? m + "?deep_link=" + this.deep_link : m), r = await D(await this.fetch(t, {
			headers: n,
			credentials: this.options.credentials ?? "same-origin"
...
async function D(e, t) {
	...
	} else if (e?.status === 401 && t) throw Error(S);
	if (e?.status === 200) {
		let t = await e.json();
		return t.dependencies?.forEach((e, t) => {
			e.id === void 0 && (e.id = t);
		}), t;
	} else if (e?.status === 401) throw Error(ce);
	throw Error(`${v}(received status ${e?.status} when fetching the app config)`);
}
async function O() {
	let { http_protocol: e, host: t } = await N(this.app_reference, this.options.token);
	try {
		if (this.options.auth) {
			let n = await k(e, t, this.options.auth, this.fetch, this.options.token, this.options.credentials);
...
async function k(e, t, n, r, i, a) {
	...
	throw c.status === 401 ? Error(S) : Error(b);
}
function A(e) {
...
}, M = /^[a-zA-Z0-9_\-\.]+\/[a-zA-Z0-9_\-\.]+$/, ve = /.*hf\.space\/{0,1}.*$/;
async function N(e, t) {
	...
	if (M.test(r)) {
		...
		if (e.status === 401 || e.status === 404) throw Error(ue(r, e.status));
		let t;
		try {
			t = (await e.json()).host;
		} catch {
	...
	if (ve.test(r)) {
		let { ws_protocol: e, http_protocol: t, host: n } = A(r);
		return {
...
function F(e, t, n) {
	...
	return Object.keys(e).forEach((i) => {
		(i === "named_endpoints" || i === "unnamed_endpoints") && (r[i] = {}, Object.entries(e[i]).forEach(([e, a]) => {
			...
			let d = (e, t, n, r) => ({
				...e,
				description: be(e?.type, n),
				type: ye(e?.type, t, n, r) || ""
			});
			r[i][e] = {
...
async function Ce() {
	...
	if (e && (n.Authorization = `Bearer ${e}`), t) try {
		...
		if (typeof window < "u" && window.gradio_api_info) r = window.gradio_api_info;
			...
			let i = P(t.root, this.api_prefix, h);
		...
		return "api" in r && (r = r.api), r.named_endpoints["/predict"] && !r.unnamed_endpoints[0] && (r.unnamed_endpoints[0] = r.named_endpoints["/predict"]), F(r, t, this.api_map);
	} catch (e) {
...
function R(e, t = 1, n = "jedec") {
	e = Math.abs(e);
	let { radix: r, unit: i } = L[n] || L.jedec, a = 0;
	for (; e >= r;) e /= r, ++a;
...
async function z(e, t, n, r) {
	...
	if (a.length) throw Error(`File(s) exceed the maximum allowed size of ${R(r || Infinity)}: ${a.map((e) => `"${e.name}"`).join(", ")}`);
...
}, Te = typeof process < "u" && process.versions && process.versions.node;
function H(e, t, n) {
	for (; n.length > 1;) {
		let t = n.shift();
...
function Oe(e) {
	if (typeof e == "string") {
		...
		if (Te) return new we("upload_file", {
			path: e,
			name: e,
			orig_path: e
		});
...
async function Ae(e, t) {
	if (!(e.config?.root || e.config?.root_url)) throw Error(w);
	await je(e, t);
}
async function je(e, t, n = []) {
	for (let r in t) t[r] instanceof we ? await Me(e, t, r) : typeof t[r] == "object" && t[r] !== null && await je(e, t[r], [...n, r]);
}
...
var Fe = async (e, t) => {
	...
	return new Promise((r) => {
		G(e, M.test(e) ? "space_name" : "subdomain", (i) => {
...
async function Re(e, t) {
	...
	t && (n.Authorization = `Bearer ${t}`);
	try {
		let t = await fetch(`https://huggingface.co/api/spaces/${e}/${ee}`, { headers: n });
...
async function Ve(e, t) {
	...
	let { http_protocol: s, host: c } = await N(e, n), l = null;
	if (o) {
		let e = await k(s, c, o, fetch, void 0, t.credentials);
		e && (l = j(e));
	}
	let u = {
...
var ot = "gradio:run-history:", K = `${ot}v2:`, st = `${ot}replay:v2:`, ct = `${ot}destination:v1:`, lt = 100, ut = 8;
function q(e, t) {
	try {
		return e();
	} catch (e) {
...
function J(e) {
	...
	try {
		return window.localStorage ? `${K}${t}${e?.username ? `:user:${encodeURIComponent(e.username)}` : ""}` : null;
	} catch {
		return null;
	}
}
function ft(e) {
	let t = J(e);
	return t ? t.replace(K, st) : null;
}
function pt(e) {
	let t = J(e);
	return t ? t.replace(K, ct) : null;
}
function mt(e) {
	let t = pt(e);
...
function _t(e) {
	let t = Object.keys(window.localStorage).filter((e) => e.startsWith(ot)), n = [...t.filter((e) => !e.startsWith(K) && !e.startsWith(st) && !e.startsWith(ct)), ...t.filter((t) => t.startsWith(K) && t !== e).sort((e, t) => gt(t) - gt(e)).slice(ut - 1)];
	for (let e of n) try {
		window.localStorage.removeItem(e), window.sessionStorage.removeItem(e.replace(K, st));
	} catch {}
}
function vt() {
...
function kt(e) {
	...
	let n = {
		...
		inputs: Y(e.inputs),
		outputs: null,
		...e.input_components ? { input_components: Y(e.input_components) } : {},
		...e.output_components ? { output_components: Y(e.output_components) } : {},
		status: "running",
		started_at: (/* @__PURE__ */ new Date()).toISOString()
	};
	return yt(e, [n, ...X(e)]), Ct(), n.id;
}
function At(e, t, n) {
	if (!t) return;
	let r = X(e), i = r.find((e) => e.id === t);
	i && (i.inputs = Y(n), yt(e, r));
}
function jt(e, t, n) {
	if (!t) return;
	let r = X(e), i = r.find((e) => e.id === t);
	if (!i) return;
	let a = !1;
	if (n.type === "data") i.outputs = Y(n.data), Z.set(t, i.outputs);
	...
	if (a) Z.has(t) && (i.outputs = Z.get(t)), Z.delete(t), Nt.delete(t);
	else if (i.streamed) {
		if (Date.now() - (Nt.get(t) ?? 0) < Mt) return;
		Nt.set(t, Date.now()), Z.delete(t);
	}
	yt(e, r);
}
var Mt = 500, Nt = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map();
function Pt(e, t) {
...
function qt(e) {
	return q(() => wt(() => q(e, void 0)), () => {});
}
//#endregion
//#region src/utils/submit.ts
function Jt(e, t = {}, n, r, i, a) {
	try {
		let { token: o } = this.options, { fetch: s, app_reference: c, config: l, session_hash: f, api_info: p, api_map: m, stream_status: h, pending_stream_messages: ee, pending_diff_streams: te, event_callbacks: ne, unclosed_events: re, post_data: g, options: v, api_prefix: y } = this, se = a || { "x-gradio-user": "api" }, b = this;
		if (!p) throw Error(de);
		if (!l) throw Error("Could not resolve app config");
		let { fn_index: x, endpoint_info: ce, dependency: S } = Xt(p, e, m, l), le = Se(t, ce), C = l.protocol ?? "ws";
		if (C === "ws") throw Error(fe);
		let ue = typeof S.api_name == "string" ? `/${S.api_name}` : e, pe = typeof S.api_name == "string" ? `/${S.api_name}` : `Function ${x}`, w = (e) => {
		...
		}, me = S.api_visibility === "public", T = l.run_history !== !1 && this.options.record_history !== !1, E = {
			app_id: l.app_id,
			username: l.username
		}, he = Lt(E), ge = {
			...se,
			...T && he.type === "bucket" ? { "x-gradio-history-bucket": he.bucket_id } : {}
		}, _e = !T || !me ? null : Wt({
			...E,
			endpoint: ue,
			api_name: pe,
			fn_index: x,
			inputs: W(le, S, l.components, "input", !0),
			input_components: S.inputs.map(w),
			output_components: S.outputs.map(w)
		}), D, O = "", k = typeof e == "number" ? "/predict" : e, A, j = null, M = !1, ve = {}, N = typeof window < "u" && typeof document < "u" ? new URLSearchParams(window.location.search).toString() : "", P = v?.events?.reduce((e, t) => (e[t] = !0, e), {}) || {};
		function F(e) {
			Kt(E, _e, e), (i || P[e.type]) && Te(e);
		}
		async function ye() {
			let e = {}, t = {};
			e = { event_id: j }, t = {
			...
			try {
				if (!l) throw Error("Could not resolve app config");
				"event_id" in t && await s(`${l.root}${y}/${ae}`, {
		...
		async function Ce(e) {
			...
			l.connect_heartbeat = n || r, await be(l), F({
				type: "render",
				data: e,
				endpoint: k,
				fn_index: x
		...
		let I = this.handle_blob(l.root, le, ce).then(async (e) => {
			let t = W(e, S, l.components, "input", !0);
			if (Gt(E, _e, t || []), A = {
				data: t || [],
				event_data: n,
				fn_index: x,
				trigger_id: r,
				...v.oauth_token && ce?.oauth_token ? { oauth_token: v.oauth_token } : {}
			}, Ee(x, l)) F({
				type: "status",
				endpoint: k,
				stage: "pending",
			...
			}), g(`${l.root}${y}/run${k.startsWith("/") ? k : `/${k}`}${N ? "?" + N : ""}`, {
				...A,
				session_hash: f
			}, ge).then(async ([e, t]) => {
				let i = e.data;
				if (t == 200) F({
					...
					endpoint: k,
					fn_index: x,
					data: W(i, S, l.components, "output", v.with_null_state),
					time: /* @__PURE__ */ new Date(),
					event_data: n,
					trigger_id: r
				}), e.render_config && await Ce(e.render_config), F({
					type: "status",
					endpoint: k,
					fn_index: x,
					stage: "complete",
				...
				let e = new URL(`${l.root}${y}/${u}?${N ? N + "&" : ""}${i}`);
				if (this.jwt && e.searchParams.set("__sign", this.jwt), D = this.stream(e), !D) return Promise.reject(/* @__PURE__ */ Error("Cannot connect to SSE endpoint: " + e.toString()));
				D.onmessage = async function(e) {
					let { type: t, status: i, data: a } = xe(JSON.parse(e.data), ve[x]);
					if (t === "update" && i && !M) F({
						type: "status",
						endpoint: k,
						fn_index: x,
						time: /* @__PURE__ */ new Date(),
						...i
					}), i.stage === "error" && (D?.close(), B());
					else if (t === "data") {
						let [e, t] = await g(`${l.root}${y}/queue/data`, {
							...A,
							session_hash: f,
							event_id: j
						}, ge);
					...
					a && (F({
						type: "data",
						time: /* @__PURE__ */ new Date(),
						data: W(a.data, S, l.components, "output", v.with_null_state),
						endpoint: k,
						fn_index: x,
						event_data: n,
						trigger_id: r
			...
			} else if (C == "sse_v1" || C == "sse_v2" || C == "sse_v2.1" || C == "sse_v3") {
				F({
					type: "status",
					stage: "pending",
					queue: !0,
				...
				return (typeof window < "u" && typeof document < "u" && window.parent != window && t && window.supports_zerogpu_headers ? De("zerogpu-headers", t) : Promise.resolve(null)).then((e) => {
					...
					return g(`${l.root}${y}/${d}?${N}`, {
						...A,
						session_hash: f
					}, t);
				}).then(async ([e, t]) => {
					if (e.event_id && (O = e.event_id), t === 503) F({
						type: "status",
						stage: "error",
						message: oe,
						queue: !0,
						...
						let t = async function(e) {
							try {
								let { type: t, status: n, data: r, original_msg: i } = xe(e, ve[x]);
								...
								if (t === "update" && n && !M) F({
									...
									return;
								} else (t === "generating" || t === "streaming") && (F({
									type: "status",
									time: /* @__PURE__ */ new Date(),
									...n,
									stage: n?.stage,
									...
									endpoint: k,
									fn_index: x
								}), r && S.connection !== "stream" && [
									"sse_v2",
									"sse_v2.1",
									"sse_v3"
								].includes(C) && nt(te, j, r));
								r && (F({
									type: "data",
									time: /* @__PURE__ */ new Date(),
									data: W(r.data, S, l.components, "output", v.with_null_state),
									endpoint: k,
									fn_index: x
								}), r.render_config && await Ce(r.render_config), M && (F({
									type: "status",
									time: /* @__PURE__ */ new Date(),
									...M,
									stage: n?.stage,
									...
									endpoint: k,
									fn_index: x
								}), B())), (n?.stage === "complete" || n?.stage === "error") && (ne[j] && delete ne[j], j in te && delete te[j], B());
							} catch (e) {
								console.error("Unexpected client exception", e), F({
									type: "status",
									stage: "error",
									...
									"sse_v2.1",
									"sse_v3"
								].includes(C) && (tt(h, b.abort_controller), h.open = !1, B());
							}
						};
						j in ee && (ee[j].forEach((e) => t(e)), delete ee[j]), ne[j] = t, re.add(j), h.open || await this.open_stream();
					}
				});
			}
		});
		...
		let L = !1, R = [], z = [];
		function B() {
			for (L = !0; z.length > 0;) z.shift()({
				value: void 0,
				done: !0
			});
		}
		function V(e) {
			z.length > 0 ? z.shift()(e) : R.push(e);
		}
		function we(e) {
			V(Yt(e)), B();
		}
		...
		function H() {
			return R.length > 0 ? Promise.resolve(R.shift()) : L ? Promise.resolve({
				value: void 0,
				done: !0
			}) : new Promise((e) => z.push(e));
		}
		let U = {
			[Symbol.asyncIterator]: () => U,
			next: H,
			throw: async (e) => (we(e), H()),
			return: async () => (B(), {
				value: void 0,
				done: !0
			}),
			cancel: ye,
			send_chunk: (e) => {
				this.post_data(`${l.root}${y}/stream/${O}`, {
					...e,
					session_hash: this.session_hash
				});
			},
			close_stream: () => {
				this.post_data(`${l.root}${y}/stream/${O}/close`, {}), B();
			},
			event_id: () => O,
			wait_for_id: async () => (await I, j)
		};
		return U;
	} catch (e) {
...
var Q = class {
	...
	constructor(e, t = { events: ["data"] }) {
		this.app_reference = e, this.deep_link = t.query_params?.deep_link || null, t.events ||= ["data"], E(t), this.options = t, this.current_payload = {}, t.cookies && (this.cookies = t.cookies), this.view_api = Ce.bind(this), this.upload_files = I.bind(this), this.handle_blob = ke.bind(this), this.post_data = Ne.bind(this), this.submit = Jt.bind(this), this.predict = Pe.bind(this), this.open_stream = et.bind(this), this.resolve_config = _e.bind(this), this.resolve_cookies = O.bind(this), this.upload = z.bind(this), this.fetch = this.fetch.bind(this), this.handle_space_success = this.handle_space_success.bind(this), this.stream = this.stream.bind(this);
	}
	async init() {
	...
	async reconnect() {
		let e = new URL(`${this.config.root}${this.api_prefix}/${g}`), t;
		try {
	...
	async refresh() {
		...
		if (!e) throw Error(v);
		this.config = e, this.api_prefix = e.api_prefix || "", this.api_map = he(e.dependencies || []);
		try {
			this.api_info = await this.view_api();
		} catch (e) {
			console.error(se + e.message);
		}
	...
	async _resolve_config() {
		let { http_protocol: e, host: t, space_id: n } = await N(this.app_reference, this.options.token), { status_callback: r } = this.options;
		n && r && await Fe(n, r);
		let i;
		try {
			let n = `${e}//${t}`;
			if (i = await this.resolve_config(n), !i) throw Error(v);
			return this.config_success(i);
		} catch (e) {
			if (n && r) G(n, M.test(n) ? "space_name" : "subdomain", this.handle_space_success);
			else throw r && r({
				status: "error",
	...
	async handle_space_success(e) {
		if (!this) throw Error(v);
		let { status_callback: t } = this.options;
		if (t && t(e), e.status === "running") try {
			if (this.config = await this._resolve_config(), this.api_prefix = this?.config?.api_prefix || "", !this.config) throw Error(v);
	...
	async component_server(e, t, n) {
		...
		try {
			let e = await this.fetch(`${o}${this.api_prefix}/${re}/`, {
				method: "POST",
				body: c,
				headers: r,
				credentials: this.options.credentials ?? "same-origin"
...
async function Zt(e, t = { events: ["data"] }) {
	return await Q.connect(e, t);
}
async function Qt(e, t) {
	return await Q.duplicate(e, t);
}
//#endregion
...
async function an(e) {
	return nn($(e, "buckets"), {}, [], (e) => Array.isArray(e?.buckets) ? e.buckets : []);
}
async function on(e, t) {
	return nn($(e, "records", { bucket: t }), {}, [], (e) => Array.isArray(e?.records) ? e.records : []);
}
function sn(e, t, n, r, i) {
	return $(e, `records/${encodeURIComponent(n)}/${encodeURIComponent(r)}/assets/${encodeURIComponent(i)}`, { bucket: t });
}
//#endregion
export { Q as Client, V as FileData, le as MISSING_CREDENTIALS_MSG, Ut as apply_run_history_replay, sn as asset_url, zt as clear_run_history, Zt as client, rn as connect_bucket, Ht as consume_run_history_replay, Bt as delete_run_history, Qt as duplicate, Oe as handle_file, en as is_valid_bucket_id, on as list_bucket_records, an as list_user_buckets, qt as on_run_history_change, Pe as predict, B as prepare_files, It as read_run_history, Lt as read_run_history_storage, dt as run_history_url, Rt as set_run_history_storage, Vt as stage_run_history_replay, Jt as submit, o as t, z as upload, I as upload_files };
